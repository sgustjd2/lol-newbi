...
- 챔피언 173개 모두 카드용 쉬운 설명과 Q/W/E/R 편집 설명을 제공한다. 카드 문구는 검수 출처를 화면에 `자료 검수 완료`로 통일하고, 내부 데이터에서만 `notebooklm`과 `public`을 구분한다.
...
- 챔피언 상세 팝업의 `대표 카운터 픽` 영역은 `data/champion-counters.json`의 16.18 LoLalytics 카운터 상위 3명을 아이콘으로 표시한다. 아이콘 버튼을 누르면 카운터 챔피언의 실제 스킬 상호작용을 바탕으로 한 `카운터 이유`와 `초보자 포인트`가 펼쳐지며, 모든 173개 챔피언의 519개 카운터 항목에 설명이 있다. 이유 문구를 다시 생성할 때는 `npm run enrich:counters`를 사용한다.
- `data/arena-augments.json`에는 173개 챔피언별 증바람 대표 증강 카드 3개가 있다. 역할별 기본 조합을 사용하되 니달리·제이스·크산테·흐웨이·유나라 등 특수 상호작용이 큰 챔피언은 개별 예시를 덮어쓴다. 증강은 패치와 제시 카드에 따라 달라지는 대표 참고값이다.
- 챔피언 목록의 `캐릭터 카드 보기` 버튼은 `#card/champion/<id>` 경로로 별도 카드 페이지를 연다. 카드에는 숫자 능력치 없이 한눈에 보기·패시브와 Q/W/E/R 쉬운 설명·카운터 3개와 이유/초보자 포인트·협곡/칼바람 추천 아이템을 표시한다. 형태별 설명은 기존 `skill.easy`를 사용하므로 니달리·제이스·엘리스·나르·케인 등의 조건이 누락되지 않는다. 인쇄 버튼과 반응형 레이아웃도 포함한다.
...
- `npm test`는 현재 35개 테스트를 통과한다.
- `data/explanations.json`의 챔피언 173개는 모두 `reviewed: true`다. 기존 NotebookLM 검수 68개와 Riot Games 공개 자료 교차검증 105개로 구성되며, 아이템 251개는 `editorial`·미검수 상태다.
...
- 이름에 " / "가 있거나 형태·강화·재시전 조건으로 효과가 달라지는 주요 스킬(현재 94개)은 `data/skill-details.json`에서 `formVariant: true`로 표시한다. 니달리·제이스·크산테 등은 `data/skill-explanations.json`에 형태별 조건을 직접 적어 두었다. 계수 파서는 여전히 한 조건의 스펠만 안전하게 매칭하므로(`scripts/sync-details.mjs`), 화면에서는 표시된 계수가 일부 조건만 확인된 값일 수 있다는 안내를 보여준다. 두 번째 형태의 모든 실제 계수를 자동 매칭하는 것은 아직 하지 않았다.
...
8. 챔피언 공개 검수는 `sourceType: "public"`과 Riot Games 공개 페이지를 사용한다. 화면에는 출처 방식 대신 `자료 검수 완료`만 표시한다.
...
| 파일 | 역할 |
|---|---|
| `index.html` | 사이트 골격, 탭, 검색, 역할군·초성 필터, 상세 dialog, 캐릭터 카드 페이지 |
| `app.js` | JSON 로드, 검색/정렬/필터, 목록 카드·상세 화면·캐릭터 카드 렌더링 |
| `style.css` | 사용자 정의 레이아웃, 롤 테마, 반응형 캐릭터 카드 스타일 |
| `seed.css` | 빌드 때 SEED 기본 토큰으로 복사 |
| `search.js` | 한글 초성 계산 및 가나다순 정렬 |
| `data/catalog.json` | Data Dragon 목록, 이미지, 스킬 원문, 태그·능력치 |
| `data/explanations.json` | 쉬운 설명과 공개/NotebookLM 검수 메타데이터 |
| `data/skill-explanations.json` | 173개 챔피언 Q/W/E/R 편집 설명 |
...
| `data/champion-counters.json` | 챔피언별 대표 카운터 3개와 스킬 상호작용을 반영한 카운터 이유·초보자 포인트, LoLalytics 출처 |
| `data/arena-augments.json` | 챔피언별 증바람 대표 증강 카드 3개와 선택 이유, 공개 추천 자료 출처 |
...
| `scripts/generate-arena-augments.mjs` | 173개 챔피언의 증바람 대표 증강 카드와 선택 이유 생성 |
...
| `scripts/verify-public-champions.mjs` | 챔피언 173개 공개 페이지·카드·QWER 전수 검증 및 공개 검수 메타데이터 반영 |
...
1. 아이템 251개를 NotebookLM 또는 공개 공식 자료로 조사하고 사람 검수로 `reviewed`를 관리한다.
2. `formVariant: true`인 94개 스킬(니달리·제이스·크산테·케인·레넥사이·엘리스 등)의 두 번째 형태 계수를 CommunityDragon 원본에서 사람이 직접 대조해 채운다. 재시전·반환·다중 적중·대상별 보정도 함께 검수한다. 현재 파서는 첫 번째로 매칭되는 스펠 하나만 계산한다.
...
- 아이템은 아직 전체가 사람 검수된 쉬운 설명 상태가 아니다.
