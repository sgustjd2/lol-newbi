...
현재 세션에서는 비공개 노트북에 로그인하지 않고 작업합니다. 기존 `data/explanations.json`에는 NotebookLM 출처로 기록된 검수 항목 68개가 있고, 챔피언 나머지 105개는 Riot Games 공개 자료를 교차검증해 반영했습니다. 새 NotebookLM 수집분은 공개 저장소에 원문이나 비공개 인용을 넣지 않습니다.
